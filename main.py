	...
	if choice0 == 3:
		...
		if choice1 == 1:
			...
			while choice2 < 1 or choice2 > 3:
				...
				if choice2 == 2:
					# make verbs
					params = raw_input("Please enter the infinitive form, perfective form, conjugation, person, number, and tense, separated by commas").strip()
					params = processInput()
					while len(params) != 6 or not(checkStr(params)):
						print("Sorry, not the right number of parameters or they aren't all strings.")
						params = raw_input("Please enter the infinitive form, perfective form, conjugation, person, number, and tense, separated by commas").strip()
					print Latin.conjugate(inf, perf, t, per, num, tense)

					choice4 = raw_input("Would you like to do more? (y/n").lower()
